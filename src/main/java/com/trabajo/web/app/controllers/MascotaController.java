package com.trabajo.web.app.controllers;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

import com.trabajo.web.app.models.Mascota;
import com.trabajo.web.app.services.MascotaService;

@CrossOrigin(origins =  "*")
@RestController
@RequestMapping("/api")
public class MascotaController {
	
	@Autowired
	private MascotaService mascotaService;
	
	@GetMapping("/mascotas")
	public List<Mascota> getMascota() {
		return mascotaService.getMascota();
	}

	@GetMapping("/mascotas/{id}")
	public Mascota getMascota(@PathVariable(name="id") Integer mascotaId) {
		Mascota mascota = mascotaService.getMascota(mascotaId);
		return mascota;
	}

	
	@PostMapping("/mascotas")
	@ResponseStatus(HttpStatus.CREATED)
	public Mascota addMascota(@RequestBody Mascota mascota) {

		mascota.setId(null);
		mascotaService.saveMascota(mascota);
		return mascota;
	}
	
		
	@PutMapping("/mascotas")
	@ResponseStatus(HttpStatus.CREATED)
	public Mascota updateMascota(@RequestBody Mascota mascota) {
		
		mascotaService.saveMascota(mascota);
		return mascota;
	}
	
	@DeleteMapping("/mascotas/{mascotaId}")
	@ResponseStatus(HttpStatus.NO_CONTENT)
	public void deleteMascota(@PathVariable int mascotaId) {
		
		mascotaService.deleteMascota(mascotaId);
	}

}
